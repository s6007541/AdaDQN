...
CORRUPTION_LIST = ['none', 'gaussian_noise', 'shot_noise', 'impulse_noise', 'speckle_noise', 
                    'gaussian_blur', 'glass_blur', 'defocus_blur', 'motion_blur', 'zoom_blur', 
                    'fog', 'frost', 'snow', 'spatter', 'contrast', 
                    'brightness', 'saturate', 'jpeg_compression', 'pixelate', 'elastic_transform']


# CORRUPTION_LIST = ['none', 'gaussian_noise', 'shot_noise', 'impulse_noise', 
#                    'glass_blur', 'defocus_blur', 'motion_blur', 'zoom_blur', 
#                     'fog', 'frost', 'snow', 'brightness', 
#                     'contrast', 'jpeg_compression', 'pixelate', 'elastic_transform']
# /////////////// Distortions ///////////////
...
def glass_blur(x, severity=1):
    ...
    for i in range(c[2]):
        for h in range(84 - c[1], c[1], -1):
            for w in range(84 - c[1], c[1], -1):
...
def defocus_blur(x, severity=1):
    ...
    channels = []
    for d in range(4):
        channels.append(cv2.filter2D(x[0][d, :, :], -1, kernel))
    # channels = np.array(channels).transpose((1, 2, 0))  # 3x32x32 -> 32x32x3

    return np.expand_dims(np.clip(channels, 0, 1) * 255, axis = 0)
...
CORRUPTION_LIST = {'none' : none, 'gaussian_noise' : gaussian_noise, 'shot_noise' : shot_noise, 'impulse_noise' : impulse_noise, 'speckle_noise' : speckle_noise, 'gaussian_blur' : gaussian_blur, 'glass_blur' : glass_blur, 'defocus_blur' : defocus_blur, 'motion_blur' : motion_blur, 'zoom_blur' : zoom_blur, 'fog' : fog, 'frost' : frost, 'snow' : snow, 'spatter' : spatter, 'contrast' : contrast, 'brightness' : brightness, 'saturate' : saturate, 'jpeg_compression' : jpeg_compression, 'pixelate': pixelate, 'elastic_transform' : elastic_transform}