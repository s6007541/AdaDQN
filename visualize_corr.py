...
def make_dataset(dir):
    ...
    #                 images.append(item)
    images = [('/mnt/sting/sorn111930/atari/atari-dqn/mspacman.jpg', 0)]
    return images
...
class DistortImageFolder(data.Dataset):
    def __init__(self, root, method, severity, transform=None, target_transform=None,
                 loader=default_loader, method_name = None):
    ...
    def __getitem__(self, index):
        ...
        save_path = f'/mnt/sting/sorn111930/atari/atari-dqn/visualize_pacman/{IMG_SIZE_}_{str(self.method_name)}_{self.severity}.jpg'
...
def plasma_fractal(mapsize=IMG_SIZE_, wibbledecay=3):
    """
...
def glass_blur(x, severity=1):
    ...
    for i in range(c[2]):
        for h in range(IMG_SIZE_ - c[1], c[1], -1):
            for w in range(IMG_SIZE_ - c[1], c[1], -1):
...
def defocus_blur(x, severity=1):
    ...
    channels = np.array(channels).transpose((1, 2, 0))  # 3xIMG_SIZE_xIMG_SIZE_ -> IMG_SIZE_xIMG_SIZE_x3
...
def motion_blur(x, severity=1):
    ...
    if x.shape != (IMG_SIZE_, IMG_SIZE_):
...
def fog(x, severity=1):
    ...
    x = np.array(x) / 255.
    max_val = x.max()
    x += c[0] * plasma_fractal(wibbledecay=c[1])[:IMG_SIZE_, :IMG_SIZE_][..., np.newaxis]
...
def frost(x, severity=1):
    ...
    x_start, y_start = np.random.randint(0, frost.shape[0] - IMG_SIZE_), np.random.randint(0, frost.shape[1] - IMG_SIZE_)
    frost = frost[x_start:x_start + IMG_SIZE_, y_start:y_start + IMG_SIZE_][..., [2, 1, 0]]
...
def snow(x, severity=1):
    ...
    x = c[6] * x + (1 - c[6]) * np.maximum(x, cv2.cvtColor(x, cv2.COLOR_RGB2GRAY).reshape(IMG_SIZE_, IMG_SIZE_, 1) * 1.5 + 0.5)
...
def pixelate(x, severity=1):
    ...
    x = x.resize((int(IMG_SIZE_ * c), int(IMG_SIZE_ * c)), PILImage.BOX)
    x = x.resize((IMG_SIZE_, IMG_SIZE_), PILImage.BOX)
...
def save_distorted(method=gaussian_noise, method_name=None):        
    for severity in range(1, 6):
        print(method.__name__, severity)
        distorted_dataset = DistortImageFolder(
            root="None",
            method=method, severity=severity,
            transform=trn.Compose([trn.Resize((IMG_SIZE_, IMG_SIZE_))]), method_name = method_name)
        distorted_dataset_loader = torch.utils.data.DataLoader(
            distorted_dataset, batch_size=1, shuffle=False, num_workers=1)
...
for method_name in d.keys():
    ...
    save_distorted(d[method_name], method_name = method_name)