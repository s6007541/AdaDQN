...
for C in "${CORRUPTIONS[@]}"; do
    for C_LVL in "${CORRUPTIONS_LEVEL[@]}"; do
        ...
        python -W ignore dqn_eval.py --tta bn_stats --checkpoint_path ${CKPTPATH} --corruption_type ${C} --game_name ${GAME} --eval_eps 1000 --corruption_level ${C_LVL} --device_id 0 --network_depth 2
